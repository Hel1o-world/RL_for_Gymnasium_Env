...
def train():
    ...
    agent.qnet.load_state_dict(torch.load("./models/cartpole_qnet_1000.pth")) #接着训练（没有数据时注释掉）
    episodes = 1001
    aver_reward = 0
    print_num = 10
    sync_num = 20
...
def test():
    ...
    agent.qnet.load_state_dict(torch.load("./models/cartpole_qnet_1000.pth"))  #加载模型参数
    agent.qnet.eval()
    total_reward = 0
    done = False
    obs, info = env.reset()
