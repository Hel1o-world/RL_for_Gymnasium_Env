...
def train():
    ...
    #agent.qnet.load_state_dict(torch.load("./models/acrobot_qnet-1000.pth")) #接着训练（没有数据时注释掉）
...
def test():
    ...
    agent.qnet.load_state_dict(torch.load("./models/acrobot_qnet-1000.pth"))  #加载模型参数
    agent.qnet.eval()
    total_reward = 0
    done = False
    obs, info = env.reset()
