...
class Acrobot:
    ...
    def get_action(self, obs):
        self.epsilon = max(self.epsilon_end, self.epsilon - 0.00001) #10000步 减少0.1
...
def train():
    ...
    #agent.qnet.load_state_dict(torch.load("./models/cartpole_actor_V1.pth")) #接着训练（没有数据时注释掉）
    episodes = 1001
    aver_reward = 0
    print_num = 10
    sync_num = 20
    ...
    for episode in range(episodes):
        ...
        if episode % 500 == 0 and episode != 0:
            torch.save(agent.qnet.state_dict(), "./models/cartpole_qnet_{}.pth".format(episode))#保存模型参数
...
def test():
    ...
    agent.qnet.load_state_dict(torch.load("../../saved_models/cartpole_actor_V1.pth"))  #加载模型参数
    agent.qnet.eval()
    total_reward = 0
    done = False
    obs, info = env.reset()
